...
def compare_values(
    helm_manifest: List[Dict[str, Any]], namespace: str, ignorable_keys: List[str]
) -> dict:
    """
...
def check_drift(release: str, namespace: str, ignorable_keys: List[str]) -> List[str]:
    """
    ...
    if not helm_manifest:
        message = "No Helm manifest found. Ensure the release exists and try again."
        logger.warning(message)
        return [message]

    return compare_values(helm_manifest, namespace, ignorable_keys)
...
def get_ignorable_keys(release: str, namespace: str) -> List[str]:
    ...
    logger.info(f"Starting Analysis for calibration...")
    ...
    for resource in helm_manifest:
        ...
        print(f"Checking drift for {kind} `{name}`...", end="\r")
...
def extract_relevant_data(
    resource: Dict[str, Any], ignorable_keys: List[str]
) -> Dict[str, Any]:
    """
    ...
    kind_ignorable_keys = {
        ...
        "Secret": [
            key.split(";", 2)[2] for key in ignorable_keys if key.startswith("Secret;")
        ],
        "Deployment": [
            key.split(";", 2)[2]
            for key in ignorable_keys
        ...
        "Service": [
            key.split(";", 2)[2] for key in ignorable_keys if key.startswith("Service;")
        ],
        "Ingress": [
            key.split(";", 2)[2] for key in ignorable_keys if key.startswith("Ingress;")
        ],
    }
